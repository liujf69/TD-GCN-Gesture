...
train_txt_path = './shrec17_dataset/HandGestureDataset_SHREC2017/train_gestures.txt' # 训练train_gestures.txt 的路径
train_txt = np.loadtxt(train_txt_path, dtype=int)


Samples_sum = train_txt.shape[0] # 样本数
# print(Samples_sum) # 1960

data_dict = []

for i in range(Samples_sum): # 遍历每一个样本
    idx_gesture = train_txt[i][0] # gesture信息
    idx_finger = train_txt[i][1] # finger信息
    idx_subject = train_txt[i][2] # subject信息
    idx_essai = train_txt[i][3] # essai信息
    label_14 = train_txt[i][4] # label_14标签
    label_28 = train_txt[i][5] # label_28标签
    T = train_txt[i][6] # 单个样本的帧数
...
with open("./shrec17_jsons/" + "train_samples.json", 'w') as t: # 将所有样本的信息保存为一个json格式文件
